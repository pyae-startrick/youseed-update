    ...
    <form action="/admin/files/store" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group d-none">
            ...
            <select class="custom-select" name="chapter_id">
                ...
                @if(!empty($chapters))
                <option value="{{$chapters[1]->id}}" selected>{{ trans('admin/main.no_chapter') }}</option>

                @else
                <option value="@if(!empty($_COOKIE['chapter_id'])) {{$_COOKIE['chapter_id']}} @endif" selected>{{ trans('admin/main.no_chapter') }}</option>
                @endif
