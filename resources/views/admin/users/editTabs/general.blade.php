            ...
            <form action="/admin/users/{{ $user->id . '/update' }}" method="Post">
                ...
                @if ($user->role_name == 'user')
                    <div class="form-group">
                        ...
                        <input type="text" name="age" class="form-control  @error('age') is-invalid @enderror"
                            value="{{ !empty($user) ? $user->age : old('age') }}" placeholder="Student Age" />
                        @error('age')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="student_id" class="form-control  @error('student_id') is-invalid @enderror"
                            value="{{ !empty($user) ? $user->student_id : old('student_id') }}" placeholder="Student ID" />
                        @error('student_id')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="parent_name"
                            ...
                            value="{{ !empty($user) ? $user->parent_name : old('parent_name') }}"
                ...
                @if ($user->role_name == 'teacher')
                    <div class="form-group mt-15 ">
                        <label class="input-label d-block">Assign an Students (<span style="color: gray;">To remove selected student you can go <a href="/admin/assigned-students" target="_blank">Teacher's Student List</a> Page and also can remove by clicking x icon and save.</span> )</label>
                        {{-- {{dd($assignStudents)}} --}}
                                ...
                                @else
                                    {{-- @if (!empty($assignStudents))
                                        @foreach ($assignStudents as $assignStudent)
                                            <option value="{{ $student->id }}" {{ ($user->id == $assignStudent->teacher_id) ? 'selected' : '' }}>
                                                {{ $student->full_name }}</option>
                                        @endforeach
                                    @endif --}}

                                    <option value="{{ $student->id }}" <?php foreach ($assignStudents as $assignStudent) {
                ...
                @if ($user->role_name != 'admin')
                    <div class="form-group">
                        ...
                        <select class="form-control @error('status') is-invalid @enderror" id="status"
                            name="status">
                            <option disabled {{ empty($user) ? 'selected' : '' }}>
                                {{ trans('admin/main.select_status') }}
                            </option>

                            @foreach (\App\User::$statuses as $status)
                                <option value="{{ $status }}"
                                    {{ !empty($user) && $user->status === $status ? 'selected' : '' }}>
