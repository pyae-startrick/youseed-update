    ...
    <div class="" style="padding: 20px 50px;">
        ...
        <div class="create-webinar-footer d-flex flex-column flex-md-row align-items-center justify-content-between mt-20 pt-15 border-top">
            <div class="d-flex align-items-center">
                ...
                @if(!empty($webinar))
                    <a href="/panel/webinars/{{ $webinar->id }}/step/{{ ($currentStep - 3) }}" class="btn btn-sm btn-primary {{ $currentStep < 5 ? 'disabled' : '' }}">{{ trans('webinars.previous') }}</a>
                @else
                    <a href="" class="btn btn-sm btn-primary disabled">{{ trans('webinars.previous') }}</a>
                @endif

                @if(!empty($webinar) and $currentStep == 1)
                <button type="button" id="getNextStep" class="btn btn-sm btn-primary ml-15" @if($currentStep >= 4)  disabled @endif>
                    Save</button>
                @else
<button type="button" id="getNextStep" class="btn btn-sm btn-primary ml-15" @if($currentStep >= 4)  disabled @endif>
                    {{ trans('webinars.next') }}</button>
                @endif
