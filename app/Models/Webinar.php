...
class Webinar extends Model implements TranslatableContract
{
    ...
    {
        // $categor = $this->category()->toArray();
        // $filters = [];
        // foreach ($categor as $category) {
        //  $options = Filter::where('category_id', $category->id)->with('options')->get();
        //  array_push($filters, $options);
        // }
        return $this->hasMany('App\Models\Filter', 'category_id', 'category_id');
    }
    ...
    public function quizzess()
    {
        return $this->hasMany('App\WebinarsQuiz', 'webinar_id', 'id');
    }
