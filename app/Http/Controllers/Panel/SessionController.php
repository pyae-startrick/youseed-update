...
class SessionController extends Controller
{
    ...
    public function joinToBigBlueButton($id)
    {
        $session = Session::where('id', $id)
            ...
            ->first();
        $title = str_replace(' ', '-',$session->title);
        ...
        $createMeeting = \Bigbluebutton::initCreateMeeting([
            ...
            'endCallbackUrl'  => 'https://youseed.com.my/course/learning/'. $title,
            'logoutUrl' => 'https://youseed.com.my/course/learning/'. $title
        ]);
        ...
        // if (!empty($session)) {
            $user = auth()->user();
            // dd($user->id, $session->creator_id);
            if ($user->id == $session->creator_id) {
                // dd('mode');
                $url = \Bigbluebutton::join([
                    'meetingID' => $session->id,
                    'userName' => $user->full_name,
                    'password' => $session->moderator_secret, //which user role want to join set password here
                    'role' => 'MODERATOR',
                    'endCallbackUrl'  => 'https://youseed.com.my/course/learning/' . $title,
                    'logoutUrl' => 'https://youseed.com.my/course/learning/' . $title 
                ]);
                ...
                // dd('user');
                    ...
                    return redirect()->to(
                        \Bigbluebutton::join([
                            'meetingID' => $session->id,
                            'userName' => $user->full_name,
                            'password' => $session->api_secret, //which user role want to join set password here
                            'role' => 'VIEWER',
                            'guestPolicy' => 'ASK_MODERATOR',
                            'endCallbackUrl'  => 'https://youseed.com.my/course/learning/' . $title,
                            'logoutUrl' => 'https://youseed.com.my/course/learning/' . $title 
