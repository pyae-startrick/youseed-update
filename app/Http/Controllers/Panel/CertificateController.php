...
class CertificateController extends Controller
{
    ...
    public function achievements(Request $request)
    {
        ...
        //     $quiz = Quiz::where('creator_id', $user->id)->where('status', 'active')->get();
        //     foreach ($quiz as $quizResult) {
        //         $results = QuizzesResult::where('quiz_id', $quizResult->id);

        //     }
        // } else {
            $results = QuizzesResult::where('user_id', $user->id);
        // }
        // dd(deepClone($results)->where('status', QuizzesResult::$failed)->count());

        // ddd(deepClone($results)->where('status', QuizzesResult::$failed)->count());
            // dd($results);
    ...
    private function makeCertificate($quizResultId)
    {
        ...
        if (!empty($quizResult) and !empty($quizResult->quiz) and $quizResult->quiz->certificate) {
            ...
            if (!empty($certificateTemplate)) {
                ...
                $img->text(dateTimeFormat($certificate->created_at, 'Y M j'), 480, 1070, function ($font) use ($certificateTemplate) {
                    $font->file(public_path('assets/default/fonts/Montserrat-Medium.ttf'));
                    $font->size($certificateTemplate->font_size);
                    $font->color($certificateTemplate->text_color);
                });
                $img->insert(public_path('assets/default/img/home/200x200.png'), 'bottom-right', 540, 90);
