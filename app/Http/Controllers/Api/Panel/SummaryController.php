...
class SummaryController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        if (!$user->isUser()) {
            ...
            $data['pending_appointments'] = count($liveRequests);
            $data['supports_count'] = count($supports);
            $data['comments_count'] = count($comments);
            $data['monthly_salesCount'] = count($monthlySales) ? $monthlySales->sum('amount') : 0;
            $data['monthly_chart'] = $this->getMonthlySalesOrPurchase($user);
            ...
            // $info['comments_count'] = $comments_count;

            $data['webinars_count'] = count($webinars);
            $data['supports_count'] = count($supports);
            $data['comments_count'] = count($comments);
            $data['reserve_meetings_count'] = count($reserveMeetings);
            $data['monthly_chart'] = $this->getMonthlySalesOrPurchase($user);
        }
